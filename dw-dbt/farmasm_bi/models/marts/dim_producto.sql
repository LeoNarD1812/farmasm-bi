with prod as ( select * from {{ ref('stg_productos') }} ),
     cat as ( select * from {{ source('raw', 'categorias') }} ),
     mar as ( select * from {{ source('raw', 'marcas') }} ),
     prov as ( select * from {{ source('raw', 'proveedores') }} )
